...
public class DocIndexMetaDataTest extends CrateDummyClusterServiceUnitTest {
    ...
    public void test_resolve_string_type_with_length_from_mappings_with_text_and_keyword_types() throws Exception {
        ...
        var keywordCol = docIndexMetaData.references().get(new ColumnIdent("keyword_col"));
        assertThat(keywordCol, is(not(nullValue())));
        assertThat(keywordCol.valueType(), is(StringType.of(10)));

        var textCol = docIndexMetaData.references().get(new ColumnIdent("text_col"));
        assertThat(textCol, is(not(nullValue())));
        assertThat(textCol.valueType(), is(StringType.of(1)));
    }
}
